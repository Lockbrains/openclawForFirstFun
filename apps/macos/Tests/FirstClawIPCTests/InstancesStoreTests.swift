...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: FirstClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: FirstClawProtocol.AnyCodable] = [
            "presence": .init([FirstClawProtocol.AnyCodable(entry)]),
        ]
        let payload = FirstClawProtocol.AnyCodable(payloadMap)
