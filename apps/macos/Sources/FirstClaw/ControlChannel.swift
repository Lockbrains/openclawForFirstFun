...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: FirstClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.firstclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: FirstClawKit.AnyCodable]()) {
                $0[$1.key] = FirstClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: FirstClawProtocol.AnyCodable?) -> [String: FirstClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: FirstClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: FirstClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: FirstClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("firstclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("firstclaw.control.agent")
}
