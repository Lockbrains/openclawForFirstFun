...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.firstclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: FirstClawPeekabooBridgeServices?
    private static var firstclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("FirstClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = FirstClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.firstclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.firstclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class FirstClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.firstclaw.peekaboo")
