...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["FIRSTCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/firstclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("firstclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("firstclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/firstclaw/ (rolling or stdout), or nil if none exist.
