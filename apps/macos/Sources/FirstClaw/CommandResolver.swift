...
enum CommandResolver {
    private static let projectRootDefaultsKey = "firstclaw.gatewayProjectRootPath"
    private static let helperName = "firstclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let firstclawEntry = root.appendingPathComponent("firstclaw.mjs").path
        if FileManager().isReadableFile(atPath: firstclawEntry) { return firstclawEntry }
        let binEntry = root.appendingPathComponent("bin/firstclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/firstclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let firstclawPaths = self.firstclawManagedPaths(home: home)
        if !firstclawPaths.isEmpty {
            extras.insert(contentsOf: firstclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + firstclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func firstclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".firstclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func firstclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectFirstClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("firstclaw.mjs").path,
            root.appendingPathComponent("bin/firstclaw.js").path,
        ]
    ...
    static func hasAnyFirstClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.firstclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func firstclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let firstclawPath = self.projectFirstClawExecutable(projectRoot: root) {
                return [firstclawPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "firstclaw", subcommand] + extraArgs
            }
            if let firstclawPath = self.firstclawExecutable(searchPaths: searchPaths) {
                return [firstclawPath, subcommand] + extraArgs
            }

            let missingEntry = """
            firstclaw entrypoint missing (looked for dist/index.js or firstclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real firstclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/firstclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v firstclaw >/dev/null 2>&1; then
          CLI="$(command -v firstclaw)"
          firstclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/firstclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/firstclaw.mjs"
            node "$PRJ/firstclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/firstclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/firstclaw.js"
            node "$PRJ/bin/firstclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent firstclaw"
          pnpm --silent firstclaw \(quotedArgs);
        else
          echo "firstclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? FirstClawConfigFile.loadDict()
