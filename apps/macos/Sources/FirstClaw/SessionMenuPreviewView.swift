...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.firstclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FirstClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FirstClawSessionPreviewEntry,
    ...
    private static func cache(payload: FirstClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FirstClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FirstClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FirstClawKit.AnyCodable]) -> [FirstClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FirstClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FirstClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FirstClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FirstClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FirstClawChatMessage) -> String? {
