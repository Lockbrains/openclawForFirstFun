...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (FirstClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: FirstClawOAuthStore.AnthropicOAuthStatus = FirstClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.firstclaw", category: "anthropic-oauth")
...
enum FirstClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let firstclawOAuthDirEnv = "FIRSTCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "FirstClaw OAuth token file not found"
            case .unreadableFile: "FirstClaw OAuth token file not readable"
            case .invalidJSON: "FirstClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in FirstClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "FirstClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.firstclawOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".firstclaw", isDirectory: true)
