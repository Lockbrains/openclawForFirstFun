...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.firstclaw/firstclaw.json. Prefer FIRSTCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.firstclaw/firstclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.firstclaw/firstclaw.json. Restart the gateway to apply."
        }
