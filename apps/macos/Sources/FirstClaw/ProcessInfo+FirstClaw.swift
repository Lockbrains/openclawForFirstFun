...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.firstclaw.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["FIRSTCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "firstclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "firstclaw.nixMode") { return true }
