...
enum DebugActions {
    private static let verboseDefaultsKey = "firstclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "firstclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = FirstClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "FirstClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FirstClawPaths.configURL
        guard
