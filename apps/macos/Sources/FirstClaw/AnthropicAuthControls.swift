...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: FirstClawOAuthStore.AnthropicOAuthStatus = FirstClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: FirstClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = FirstClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = FirstClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try FirstClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. FirstClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: FirstClawOAuthStore.AnthropicOAuthStatus,
