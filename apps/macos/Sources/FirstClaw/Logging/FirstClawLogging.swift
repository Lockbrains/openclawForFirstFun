...
enum FirstClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = FirstClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = FirstClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        FirstClawLogging.bootstrapIfNeeded()
        let label = FirstClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct FirstClawOSLogHandler: LogHandler {
...
struct FirstClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = FirstClawLogging.parseLabel(self.label)
