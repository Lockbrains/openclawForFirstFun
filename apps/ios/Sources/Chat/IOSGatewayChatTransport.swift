...
struct IOSGatewayChatTransport: FirstClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> FirstClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(FirstClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> FirstClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(FirstClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [FirstClawChatAttachmentPayload]) async throws -> FirstClawChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [FirstClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(FirstClawChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(FirstClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FirstClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FirstClawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FirstClawAgentEventPayload.self)
                        {
