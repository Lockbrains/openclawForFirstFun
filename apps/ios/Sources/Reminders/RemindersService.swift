...
final class RemindersService: RemindersServicing {
    func list(params: FirstClawRemindersListParams) async throws -> FirstClawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[FirstClawReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return FirstClawReminderPayload(
        ...
        return FirstClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: FirstClawRemindersAddParams) async throws -> FirstClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = FirstClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return FirstClawRemindersAddPayload(reminder: payload)
    }
