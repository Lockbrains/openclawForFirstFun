...
final class MotionService: MotionServicing {
    func activities(params: FirstClawMotionActivityParams) async throws -> FirstClawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[FirstClawMotionActivityEntry], Error>) in
        ...
        return FirstClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: FirstClawPedometerParams) async throws -> FirstClawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<FirstClawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = FirstClawPedometerPayload(
