...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> FirstClawNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> FirstClawNetworkStatusPayload {
        let status: FirstClawNetworkPathStatus = switch path.status {
        ...
        var interfaces: [FirstClawNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return FirstClawNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> FirstClawNetworkStatusPayload {
        FirstClawNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
