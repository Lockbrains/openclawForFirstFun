...
final class ContactsService: ContactsServicing {
    ...
    func search(params: FirstClawContactsSearchParams) async throws -> FirstClawContactsSearchPayload {
        ...
        return FirstClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: FirstClawContactsAddParams) async throws -> FirstClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return FirstClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return FirstClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> FirstClawContactPayload {
        FirstClawContactPayload(
