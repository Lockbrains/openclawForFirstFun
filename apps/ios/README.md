...
This is an **alpha** iOS app that connects to an FirstClaw Gateway as a `role: node`.
...
3. On iOS: FirstClaw → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project FirstClaw.xcodeproj -scheme FirstClaw -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/FirstClawKit` contains the shared transport/types used by the iOS app.
