...
import Foundation
import Testing
@testable import FirstClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, FirstClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [FirstClawChatHistoryPayload]
    private let sessionsResponses: [FirstClawChatSessionsListResponse]

    private let stream: AsyncStream<FirstClawChatTransportEvent>
    private let continuation: AsyncStream<FirstClawChatTransportEvent>.Continuation

    init(
        historyResponses: [FirstClawChatHistoryPayload],
        sessionsResponses: [FirstClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<FirstClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> FirstClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? FirstClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [FirstClawChatAttachmentPayload]) async throws -> FirstClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return FirstClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> FirstClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? FirstClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: FirstClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = FirstClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = FirstClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { FirstClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = FirstClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = FirstClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = FirstClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { FirstClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
