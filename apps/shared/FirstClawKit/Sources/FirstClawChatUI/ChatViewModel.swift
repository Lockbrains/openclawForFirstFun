...
private let chatUILogger = Logger(subsystem: "ai.firstclaw", category: "FirstClawChatUI")

@MainActor
@Observable
public final class FirstClawChatViewModel {
    public private(set) var messages: [FirstClawChatMessage] = []
    ...
    public var attachments: [FirstClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [FirstClawChatPendingToolCall] = []
    public private(set) var sessions: [FirstClawChatSessionEntry] = []
    private let transport: any FirstClawChatTransport
    ...
    private var pendingToolCallsById: [String: FirstClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any FirstClawChatTransport) {
    ...
    public var sessionChoices: [FirstClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [FirstClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [FirstClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: FirstClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [FirstClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: FirstClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [FirstClawChatMessage]) -> [FirstClawChatMessage] {
        var result: [FirstClawChatMessage] = []
    ...
    private static func dedupeKey(for message: FirstClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [FirstClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> FirstClawChatAttachmentPayload in
            FirstClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> FirstClawChatSessionEntry {
        FirstClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: FirstClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: FirstClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: FirstClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = FirstClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> FirstClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
