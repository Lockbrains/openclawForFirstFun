...
enum FirstClawChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FirstClawChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FirstClawChatTheme.onboardingAssistantBubble"),
    ...
    static var background: some View {
        ...
        ZStack {
            ...
            RadialGradient(
                colors: [
                    Color(nsColor: .systemCyan).opacity(0.14),
                    .clear,
                ],
                center: .topLeading,
                startRadius: 40,
...
enum FirstClawPlatformImageFactory {
    static func image(_ image: FirstClawPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
