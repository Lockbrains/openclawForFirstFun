...
This directory contains hooks that ship with FirstClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.firstclaw.ai/hooks/soul-evil
...
Hooks can be configured in `~/.firstclaw/firstclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'firstclaw.*gateway' && pnpm firstclaw gateway`
3. Enable the hook: `firstclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.firstclaw.ai/hooks
