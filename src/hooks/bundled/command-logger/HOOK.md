...
homepage: https://docs.firstclaw.ai/hooks#command-logger
metadata:
  {
    "firstclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.firstclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.firstclaw/logs/commands.log ~/.firstclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/firstclaw`:
   ```
   /home/username/.firstclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
