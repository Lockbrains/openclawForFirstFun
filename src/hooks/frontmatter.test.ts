...
`;
    ...
    expect(parsed.firstclaw.emoji).toBe("💾");
    expect(parsed.firstclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.firstclaw.emoji).toBe("📝");
    expect(parsed.firstclaw.events).toEqual(["command"]);
    expect(parsed.firstclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.firstclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"firstclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"firstclaw": {"events": ["test"]}}');
  });
...
describe("resolveFirstClawMetadata", () => {
  it("extracts firstclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveFirstClawMetadata(frontmatter);
  ...
  it("returns undefined when firstclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        firstclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with FirstClaw" },
            { id: "npm", kind: "npm", package: "@firstclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@firstclaw/hook");
  });
...
`;
    ...
    const firstclaw = resolveFirstClawMetadata(frontmatter);
    expect(firstclaw).toBeDefined();
    expect(firstclaw?.emoji).toBe("💾");
    expect(firstclaw?.events).toEqual(["command:new"]);
    expect(firstclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(firstclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const firstclaw = resolveFirstClawMetadata(frontmatter);
    expect(firstclaw?.emoji).toBe("disk");
    expect(firstclaw?.events).toEqual(["command:new"]);
  });
});
