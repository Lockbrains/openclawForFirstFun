...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "firstclaw",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/firstclaw.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("firstclaw");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "firstclaw");
