...
async function withTempStateDir<T>(fn: (stateDir: string) => Promise<T>) {
  const previous = process.env.FIRSTCLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-pairing-"));
  process.env.FIRSTCLAW_STATE_DIR = dir;
  try {
    return await fn(dir);
  } finally {
    if (previous === undefined) {
      ...
      process.env.FIRSTCLAW_STATE_DIR = previous;
    }
