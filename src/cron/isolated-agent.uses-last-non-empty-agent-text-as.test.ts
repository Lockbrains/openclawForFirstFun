...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "firstclaw-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".firstclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<FirstClawConfig> = {},
): FirstClawConfig {
  const base: FirstClawConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".firstclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
