...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "firstclaw-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "firstclaw-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "firstclaw-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "firstclaw-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.firstclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "firstclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers FIRSTCLAW_HOME over HOME", () => {
    vi.stubEnv("FIRSTCLAW_HOME", "/srv/firstclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/firstclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $FIRSTCLAW_HOME prefix when FIRSTCLAW_HOME is set", () => {
    vi.stubEnv("FIRSTCLAW_HOME", "/srv/firstclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomePath(`${path.resolve("/srv/firstclaw-home")}/.firstclaw/firstclaw.json`),
    ).toBe("$FIRSTCLAW_HOME/.firstclaw/firstclaw.json");
...
describe("shortenHomeInString", () => {
  it("uses $FIRSTCLAW_HOME replacement when FIRSTCLAW_HOME is set", () => {
    vi.stubEnv("FIRSTCLAW_HOME", "/srv/firstclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(
        `config: ${path.resolve("/srv/firstclaw-home")}/.firstclaw/firstclaw.json`,
      ),
    ).toBe("config: $FIRSTCLAW_HOME/.firstclaw/firstclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/firstclaw")).toBe(path.resolve(os.homedir(), "firstclaw"));
  });
  ...
  it("prefers FIRSTCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("FIRSTCLAW_HOME", "/srv/firstclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/firstclaw")).toBe(path.resolve("/srv/firstclaw-home", "firstclaw"));
