...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.FIRSTCLAW_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.FIRSTCLAW_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.FIRSTCLAW_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[firstclaw] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[firstclaw] ${parsed.error}`);
    process.exit(2);
  }
