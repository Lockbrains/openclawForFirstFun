...
const requireAgentDir = () => {
  const agentDir = process.env.FIRSTCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FIRSTCLAW_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.FIRSTCLAW_STATE_DIR;
  const previousAgentDir = process.env.FIRSTCLAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.FIRSTCLAW_AGENT_DIR;
    } else {
      process.env.FIRSTCLAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.FIRSTCLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under FIRSTCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-oauth-"));
    process.env.FIRSTCLAW_STATE_DIR = tempStateDir;
    process.env.FIRSTCLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FIRSTCLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FIRSTCLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-minimax-"));
    process.env.FIRSTCLAW_STATE_DIR = tempStateDir;
    process.env.FIRSTCLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FIRSTCLAW_AGENT_DIR;
