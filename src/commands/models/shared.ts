...
export async function updateConfig(
  mutator: (cfg: FirstClawConfig) => FirstClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FirstClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FirstClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("firstclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
