...
import { readConfigFileSnapshot, type FirstClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: FirstClawConfig, model: string): FirstClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("firstclaw plugins install")}\`.`,
    );
  }
