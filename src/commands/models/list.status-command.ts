...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveFirstClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("firstclaw models auth setup-token")}\` or \`${formatCliCommand("firstclaw configure")}\`.`
          : `Run \`${formatCliCommand("firstclaw configure")}\` or set an API key env var.`;
