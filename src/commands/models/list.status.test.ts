...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveFirstClawAgentDir: vi.fn().mockReturnValue("/tmp/firstclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/firstclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/firstclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveFirstClawAgentDir: mocks.resolveFirstClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveFirstClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/firstclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/firstclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/firstclaw-agent-custom");
