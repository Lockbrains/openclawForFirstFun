...
export function listAgentEntries(cfg: FirstClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: FirstClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: FirstClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: FirstClawConfig): AgentSummary[] {
