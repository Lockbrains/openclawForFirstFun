...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.FIRSTCLAW_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "firstclaw");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.FIRSTCLAW_STATE_DIR = stateDir;
    process.env.FIRSTCLAW_CONFIG_PATH = path.join(stateDir, "firstclaw.json");
