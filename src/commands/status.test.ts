...
beforeAll(() => {
  previousProfile = process.env.FIRSTCLAW_PROFILE;
  process.env.FIRSTCLAW_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.FIRSTCLAW_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/firstclaw-root.js", () => ({
  resolveFirstClawPackageRoot: vi.fn().mockResolvedValue("/tmp/firstclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/firstclaw/pnpm-lock.yaml",
      markerPath: "/tmp/firstclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("FirstClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("firstclaw status --all") ||
          l.includes("firstclaw --profile isolated status --all") ||
          l.includes("firstclaw status --all") ||
          l.includes("firstclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FIRSTCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
