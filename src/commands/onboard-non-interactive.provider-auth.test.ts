...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.FIRSTCLAW_GATEWAY_PASSWORD,
    disableConfigCache: process.env.FIRSTCLAW_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("FIRSTCLAW_STATE_DIR", prev.stateDir);
  restoreEnvVar("FIRSTCLAW_CONFIG_PATH", prev.configPath);
  restoreEnvVar("FIRSTCLAW_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("FIRSTCLAW_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("FIRSTCLAW_SKIP_CRON", prev.skipCron);
  restoreEnvVar("FIRSTCLAW_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("FIRSTCLAW_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("FIRSTCLAW_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("FIRSTCLAW_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  process.env.FIRSTCLAW_DISABLE_CONFIG_CACHE = "1";
  delete process.env.FIRSTCLAW_GATEWAY_TOKEN;
  delete process.env.FIRSTCLAW_GATEWAY_PASSWORD;

  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), prefix));
  const configPath = path.join(tempHome, "firstclaw.json");
  process.env.HOME = tempHome;
  process.env.FIRSTCLAW_STATE_DIR = tempHome;
  process.env.FIRSTCLAW_CONFIG_PATH = configPath;
  vi.resetModules();
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("firstclaw-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("firstclaw-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("firstclaw-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("firstclaw-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("firstclaw-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("firstclaw-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
