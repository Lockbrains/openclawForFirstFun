...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.FIRSTCLAW_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: FirstClawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.FIRSTCLAW_DEBUG_HEALTH);
    if (opts.verbose) {
