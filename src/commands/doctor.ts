...
function resolveMode(cfg: FirstClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("FirstClaw doctor");

  const root = await resolveFirstClawPackageRoot({
  ...
  let cfg: FirstClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("firstclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("firstclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("firstclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("firstclaw doctor --fix")}" to apply changes.`);
  }
