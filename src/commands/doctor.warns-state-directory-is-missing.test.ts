...
beforeEach(() => {
  ...
  resolveFirstClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.FIRSTCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.FIRSTCLAW_UPDATE_IN_PROGRESS;
  process.env.FIRSTCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "firstclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.FIRSTCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.FIRSTCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.FIRSTCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveFirstClawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadFirstClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/firstclaw.json",
    createConfigIO,
...
vi.mock("../infra/firstclaw-root.js", () => ({
  resolveFirstClawPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "firstclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.FIRSTCLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when FIRSTCLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/firstclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.FIRSTCLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
