...
import type { FirstClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: FirstClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("firstclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("firstclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("firstclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("firstclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("firstclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
