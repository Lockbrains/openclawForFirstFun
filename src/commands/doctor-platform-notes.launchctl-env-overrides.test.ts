...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "FIRSTCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("FIRSTCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv FIRSTCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("FIRSTCLAW_GATEWAY_PASSWORD");
  });
