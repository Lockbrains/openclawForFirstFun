...
const SEARCH_TOOL = "https://docs.firstclaw.ai/mcp.SearchFirstClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.firstclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('firstclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.firstclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('firstclaw docs "your query"')}`);
    }
    return;
  }
