...
${formatHelpExamples([
  ['firstclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['firstclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'firstclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'firstclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['firstclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.firstclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.firstclaw.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'firstclaw agents set-identity --agent main --name "FirstClaw" --emoji "🦞"',
    "Set name + emoji.",
  ],
  ["firstclaw agents set-identity --agent main --avatar avatars/firstclaw.png", "Set avatar path."],
  [
    "firstclaw agents set-identity --workspace ~/.firstclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "firstclaw agents set-identity --identity-file ~/.firstclaw/workspace/IDENTITY.md --agent main",
