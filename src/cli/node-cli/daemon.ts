...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("firstclaw node install"),
    formatCliCommand("firstclaw node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("firstclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    FIRSTCLAW_LOG_PREFIX: baseEnv.FIRSTCLAW_LOG_PREFIX ?? "node",
