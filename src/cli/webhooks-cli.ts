...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.firstclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + FirstClaw hooks")
    ...
    .option("--hook-url <url>", "FirstClaw hook URL")
    .option("--hook-token <token>", "FirstClaw hook token")
