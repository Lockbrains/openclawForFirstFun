...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "firstclaw-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      FIRSTCLAW_NO_RESPAWN: "1",
      FIRSTCLAW_STATE_DIR: stateDir,
      FIRSTCLAW_SKIP_CHANNELS: "1",
      FIRSTCLAW_SKIP_GMAIL_WATCHER: "1",
      FIRSTCLAW_SKIP_CRON: "1",
      FIRSTCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
      FIRSTCLAW_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "firstclaw-entry-bootstrap.mjs");
