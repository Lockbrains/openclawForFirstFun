...
export function applyCliProfileEnv(params: {
  ...
  env.FIRSTCLAW_PROFILE = profile;

  const stateDir = env.FIRSTCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.FIRSTCLAW_STATE_DIR?.trim()) {
    env.FIRSTCLAW_STATE_DIR = stateDir;
  }

  if (!env.FIRSTCLAW_CONFIG_PATH?.trim()) {
    env.FIRSTCLAW_CONFIG_PATH = path.join(stateDir, "firstclaw.json");
  }

  if (profile === "dev" && !env.FIRSTCLAW_GATEWAY_PORT?.trim()) {
    env.FIRSTCLAW_GATEWAY_PORT = "19001";
  }
}
