...
import type {
  FirstClawPluginDefinition,
  FirstClawPluginModule,
  PluginDiagnostic,
  PluginLogger,
} from "./types.js";
...
import { discoverFirstClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FirstClawPluginDefinition;
  register?: FirstClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as FirstClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as FirstClawPluginDefinition;
...
export function loadFirstClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverFirstClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "firstclaw/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: FirstClawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as FirstClawPluginModule;
    } catch (err) {
