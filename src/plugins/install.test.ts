...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `firstclaw-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.firstclaw/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@firstclaw/voice-call",
        version: "0.0.1",
        firstclaw: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without firstclaw.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@firstclaw/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("firstclaw.extensions");
  });
