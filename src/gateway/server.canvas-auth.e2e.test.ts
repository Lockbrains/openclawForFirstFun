...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.FIRSTCLAW_CONFIG_PATH;
  const prevDisableCache = process.env.FIRSTCLAW_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "firstclaw-canvas-auth-test-"));
  const configPath = path.join(dir, "firstclaw.json");
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.FIRSTCLAW_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.FIRSTCLAW_DISABLE_CONFIG_CACHE;
    } else {
      process.env.FIRSTCLAW_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
