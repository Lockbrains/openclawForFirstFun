...
export function resolveHooksConfig(cfg: FirstClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: FirstClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-firstclaw-token"] === "string"
      ? req.headers["x-firstclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
