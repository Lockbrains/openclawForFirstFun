...
import { createFirstClawTools } from "../agents/firstclaw-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.FIRSTCLAW_GATEWAY_PORT;
  prevGatewayToken = process.env.FIRSTCLAW_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.FIRSTCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.FIRSTCLAW_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.FIRSTCLAW_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.FIRSTCLAW_GATEWAY_TOKEN;
  } else {
    process.env.FIRSTCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createFirstClawTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
