    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "firstclaw", messages: [{ role: "user", content: "hi" }] },
          { "x-firstclaw-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-firstclaw-agent-id": "beta",
            "x-firstclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
