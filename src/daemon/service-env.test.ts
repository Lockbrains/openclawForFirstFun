...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.FIRSTCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.FIRSTCLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.FIRSTCLAW_SERVICE_MARKER).toBe("firstclaw");
    expect(env.FIRSTCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.FIRSTCLAW_SERVICE_VERSION).toBe("string");
    expect(env.FIRSTCLAW_SYSTEMD_UNIT).toBe("firstclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.FIRSTCLAW_LAUNCHD_LABEL).toBe("ai.firstclaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", FIRSTCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.FIRSTCLAW_SYSTEMD_UNIT).toBe("firstclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.FIRSTCLAW_LAUNCHD_LABEL).toBe("ai.firstclaw.work");
    }
  });
});
