...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.FIRSTCLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.FIRSTCLAW_STATE_DIR;
  const configPath = env.FIRSTCLAW_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    FIRSTCLAW_PROFILE: profile,
    FIRSTCLAW_STATE_DIR: stateDir,
    FIRSTCLAW_CONFIG_PATH: configPath,
    FIRSTCLAW_GATEWAY_PORT: String(port),
    FIRSTCLAW_GATEWAY_TOKEN: token,
    FIRSTCLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    FIRSTCLAW_SYSTEMD_UNIT: systemdUnit,
    FIRSTCLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    FIRSTCLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    FIRSTCLAW_STATE_DIR: stateDir,
    FIRSTCLAW_CONFIG_PATH: configPath,
    FIRSTCLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    FIRSTCLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    FIRSTCLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    FIRSTCLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    FIRSTCLAW_LOG_PREFIX: "node",
    FIRSTCLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    FIRSTCLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    FIRSTCLAW_SERVICE_VERSION: VERSION,
  };
}
