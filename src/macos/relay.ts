...
async function main() {
  ...
  const { ensureFirstClawCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[firstclaw] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
