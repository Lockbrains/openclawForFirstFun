...
declare const __FIRSTCLAW_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __FIRSTCLAW_VERSION__ === "string" && __FIRSTCLAW_VERSION__) ||
  process.env.FIRSTCLAW_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `firstclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.FIRSTCLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.FIRSTCLAW_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[firstclaw] Gateway daemon failed:",
