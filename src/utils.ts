...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".firstclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.FIRSTCLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$FIRSTCLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via FIRSTCLAW_STATE_DIR.
