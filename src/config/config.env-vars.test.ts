...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.firstclaw/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          FIRSTCLAW_STATE_DIR: path.join(home, ".firstclaw"),
          CLAWDBOT_STATE_DIR: undefined,
          FIRSTCLAW_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          FIRSTCLAW_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
