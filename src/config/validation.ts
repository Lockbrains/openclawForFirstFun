...
import type { FirstClawConfig, ConfigValidationIssue } from "./types.js";
...
import { FirstClawSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: FirstClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: FirstClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = FirstClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as FirstClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as FirstClawConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as FirstClawConfig)),
    ),
  };
}
