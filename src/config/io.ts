...
import type { FirstClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareFirstClawVersions } from "./version.js";
...
function coerceConfig(value: unknown): FirstClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as FirstClawConfig;
}
...
function stampConfigVersion(cfg: FirstClawConfig): FirstClawConfig {
...
function warnIfConfigFromFuture(cfg: FirstClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareFirstClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer FirstClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: FirstClawConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): FirstClawConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as FirstClawConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as FirstClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: FirstClawConfig) {
    clearConfigCache();
...
// module scope. `FIRSTCLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FIRSTCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FIRSTCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): FirstClawConfig {
...
export async function writeConfigFile(cfg: FirstClawConfig): Promise<void> {
  clearConfigCache();
