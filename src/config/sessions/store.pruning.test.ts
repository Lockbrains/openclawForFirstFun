...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real firstclaw.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.FIRSTCLAW_SESSION_CACHE_TTL_MS;
    process.env.FIRSTCLAW_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.FIRSTCLAW_SESSION_CACHE_TTL_MS;
    } else {
      process.env.FIRSTCLAW_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
