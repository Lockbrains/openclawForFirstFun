...
const ETC_FIRSTCLAW_DIR = path.join(ROOT_DIR, "etc", "firstclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "firstclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcFirstClawPath(...parts: string[]) {
  return path.join(ETC_FIRSTCLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcFirstClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "firstclaw.json"))).toEqual({
      shared: true,
    });
  });
});
