...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-firstclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-firstclaw#general");
  });
  ...
  it("derives session transcripts dir from FIRSTCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { FIRSTCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.FIRSTCLAW_STATE_DIR = "/custom/state";
    try {
