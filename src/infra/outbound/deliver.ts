...
import { guardOutput } from "../../security/output-guard.js";
import { throwIfAborted } from "./abort.js";
import { normalizeReplyPayloadsForDelivery } from "./payloads.js";

const deliverLog = createSubsystemLogger("outbound/deliver");

export type { NormalizedOutboundPayload } from "./payloads.js";
export { normalizeOutboundPayloads } from "./payloads.js";

// FirstClaw: Simplified OutboundSendDeps - iMessage + stubs for removed channels
type SendStub = (...args: unknown[]) => Promise<never>;
export type OutboundSendDeps = {
  sendWhatsApp?: SendStub;
  sendTelegram?: SendStub;
  sendDiscord?: SendStub;
  sendSlack?: SendStub;
  sendSignal?: SendStub;
...
export async function deliverOutboundPayloads(params: {
  ...
  for (const payload of normalizedPayloads) {
    // FirstClaw: Output Guard - redact sensitive content before delivery
    const rawText = payload.text ?? "";
    const guardResult = guardOutput(rawText);
    if (guardResult.redacted) {
      deliverLog.warn("Output guard redacted sensitive content", {
        channel,
        to,
        matchCount: guardResult.matches.length,
        patterns: guardResult.matches.map((m) => m.pattern),
      });
      payload.text = guardResult.text;
    }
