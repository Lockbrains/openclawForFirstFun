...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "firstclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("firstclaw doctor", doctorArgv, gitRoot, { FIRSTCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
