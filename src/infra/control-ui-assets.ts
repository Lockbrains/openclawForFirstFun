...
import { resolveFirstClawPackageRoot, resolveFirstClawPackageRootSync } from "./firstclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveFirstClawPackageRoot({ argv1: normalized });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "firstclaw" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveFirstClawPackageRootSync({
    argv1,
  ...
  if (argv1Dir) {
    // firstclaw.mjs or dist/<bundle>.js
