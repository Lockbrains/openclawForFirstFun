...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.FIRSTCLAW_MDNS_HOSTNAME = "test-host";
    process.env.FIRSTCLAW_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("firstclaw-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("firstclaw (FirstClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("firstclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("firstclaw.local");
