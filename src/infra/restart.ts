...
export function triggerFirstClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.FIRSTCLAW_SYSTEMD_UNIT,
        process.env.FIRSTCLAW_PROFILE,
      );
  ...
  const label =
    process.env.FIRSTCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.FIRSTCLAW_PROFILE);
