...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.FIRSTCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*firstclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "firstclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "firstclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "firstclaw-gw");
  ...
  records.push(`_firstclaw-gw._tcp IN PTR ${instanceLabel}._firstclaw-gw._tcp`);
  records.push(`${instanceLabel}._firstclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._firstclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; firstclaw-content-hash: ${contentHash}\n${contentBody}`;
}
