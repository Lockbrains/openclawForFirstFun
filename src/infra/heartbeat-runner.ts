...
function resolveActiveHoursTimezone(cfg: FirstClawConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: FirstClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: FirstClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: FirstClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: FirstClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: FirstClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: FirstClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: FirstClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
