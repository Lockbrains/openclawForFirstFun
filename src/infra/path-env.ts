...
function candidateBinDirs(opts: EnsureFirstClawPathOpts): string[] {
  ...
  // Bundled macOS app: `firstclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "firstclaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/firstclaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "firstclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `firstclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFirstClawCliOnPath(opts: EnsureFirstClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FIRSTCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.FIRSTCLAW_PATH_BOOTSTRAPPED = "1";
