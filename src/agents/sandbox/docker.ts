...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "firstclaw.sandbox=1");
  args.push("--label", `firstclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `firstclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `firstclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("firstclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`firstclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`firstclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("firstclaw sandbox recreate --all");
}
