...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-embedded-agent-"));
...
const ensureModels = (cfg: FirstClawConfig) => ensureFirstClawModelsJson(cfg, agentDir) as unknown;
