...
describe("createFirstClawCodingTools", () => {
  it("preserves action enums in normalized schemas", () => {
    ...
    const existingToolNames = toolNames.filter((name) => defaultTools.some((t) => t.name === name));
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createFirstClawCodingTools({
      config,
    ...
    const anthropicTools = createFirstClawCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createFirstClawCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createFirstClawCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
