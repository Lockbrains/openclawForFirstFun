...
import { createFirstClawTools } from "./firstclaw-tools.js";
import { __testing, createFirstClawCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";

const defaultTools = createFirstClawCodingTools();

describe("createFirstClawCodingTools", () => {
  ...
  it("flattens anyOf-of-literals to enum for provider compatibility", () => {
    ...
    expect(action?.enum).toContain("present");

    const outputFormat = parameters?.properties?.outputFormat as
      | {
          type?: unknown;
          enum?: unknown[];
          anyOf?: unknown[];
      ...
      | undefined;
    expect(outputFormat?.type).toBe("string");
    expect(outputFormat?.anyOf).toBeUndefined();
    expect(outputFormat?.enum).toContain("png");
  });
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createFirstClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createFirstClawCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "firstclaw-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "firstclaw-outside.txt");
