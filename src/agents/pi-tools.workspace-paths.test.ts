...
import { createFirstClawCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("firstclaw-ws-", async (workspaceDir) => {
      await withTempDir("firstclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createFirstClawCodingTools({ workspaceDir });
    ...
    await withTempDir("firstclaw-ws-", async (workspaceDir) => {
      await withTempDir("firstclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello firstclaw");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("firstclaw-ws-", async (workspaceDir) => {
      const tools = createFirstClawCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("firstclaw-ws-", async (workspaceDir) => {
      await withTempDir("firstclaw-override-", async (overrideDir) => {
        const tools = createFirstClawCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("firstclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("firstclaw-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "firstclaw-sbx-test",
        ...
        const tools = createFirstClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
