...
type WebSearchConfig = NonNullable<FirstClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: FirstClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("firstclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.firstclaw.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://firstclaw.ai",
      "X-Title": "FirstClaw Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
