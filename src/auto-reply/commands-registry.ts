...
export function isCommandEnabled(cfg: FirstClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: FirstClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as FirstClawConfig),
...
export function getCommandDetection(_cfg?: FirstClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: FirstClawConfig) {
