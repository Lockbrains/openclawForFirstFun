...
metadata:
  { "firstclaw": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (FirstClaw)
...
```bash
SOCKET_DIR="${FIRSTCLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/firstclaw-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/firstclaw.sock"
SESSION=firstclaw-python
...
- Use `FIRSTCLAW_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$FIRSTCLAW_TMUX_SOCKET_DIR/firstclaw.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `FIRSTCLAW_TMUX_SOCKET_DIR`).
