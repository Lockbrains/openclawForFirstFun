---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for FirstClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, FirstClaw cron scheduling for periodic checks, or version status checks on a machine running FirstClaw (laptop, workstation, Pi, VPS).
---

# FirstClaw Host Hardening

## Overview

Assess and harden the host running FirstClaw, then align it to a user-defined risk tolerance without breaking access. Use FirstClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FirstClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. FirstClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FirstClaw can access sensitive data.
...
### 2) Run FirstClaw security audits (read-only)

As part of the default read-only checks, run `firstclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `firstclaw security audit` (faster, non-probing)
2. `firstclaw security audit --json` (structured output)

Offer to apply FirstClaw safe defaults (numbered):

1. `firstclaw security audit --fix`

Be explicit that `--fix` only tightens FirstClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FirstClaw version/update status (read-only)

As part of the default read-only checks, run `firstclaw update status`.
...
- Credential hygiene notes (location of FirstClaw creds, prefer disk encryption)
...
- FirstClaw security audit (re-run)
...
After FirstClaw install or first hardening pass, run at least one baseline audit and version check:

- `firstclaw security audit`
- `firstclaw security audit --deep`
- `firstclaw update status`

Ongoing monitoring is recommended. Use the FirstClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `firstclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `firstclaw update status`
...
Before creating, `firstclaw cron list` and match on exact `name`. If found, `firstclaw cron edit <id> ...`.
If not found, `firstclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `firstclaw update status` (preferred for source checkouts and channels)
2. `npm view firstclaw version` (published npm version)

## FirstClaw command accuracy

Use only supported commands and flags:

- `firstclaw security audit [--deep] [--fix] [--json]`
- `firstclaw status` / `firstclaw status --deep`
- `firstclaw health --json`
- `firstclaw update status`
- `firstclaw cron add|list|runs|run`

Do not invent CLI flags or imply FirstClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FirstClaw compaction:
