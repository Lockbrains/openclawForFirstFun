...
#
# Suggested cron: */30 * * * * /home/admin/firstclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FirstClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., firstclaw-alerts)
...
STATE_FILE="$HOME/.firstclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FirstClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use firstclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FirstClaw to $NOTIFY_PHONE..."
            firstclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FirstClaw is down. Run: ssh l36 '~/firstclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
