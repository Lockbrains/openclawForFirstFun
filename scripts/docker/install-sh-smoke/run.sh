...
INSTALL_URL="${FIRSTCLAW_INSTALL_URL:-https://firstclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${FIRSTCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FIRSTCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="firstclaw"
PACKAGE_NAME="${FIRSTCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FIRSTCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FIRSTCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${FIRSTCLAW_INSTALL_LATEST_OUT:-}"
fi
