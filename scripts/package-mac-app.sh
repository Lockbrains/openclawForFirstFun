...
# Build and bundle FirstClaw into a minimal .app we can open.
# Outputs to dist/FirstClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FirstClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="FirstClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.firstclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/firstclaw/firstclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FirstClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FirstClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FirstClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FirstClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FirstClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FirstClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FirstClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FirstClaw/Resources/FirstClaw.icns" "$APP_ROOT/Contents/Resources/FirstClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FirstClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FirstClawKit resources"
FIRSTCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FirstClawKit_FirstClawKit.bundle"
if [ -d "$FIRSTCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FirstClawKit_FirstClawKit.bundle"
  cp -R "$FIRSTCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FirstClawKit_FirstClawKit.bundle"
else
  echo "WARN: FirstClawKit resource bundle not found at $FIRSTCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FirstClaw"
killall -q FirstClaw 2>/dev/null || true
