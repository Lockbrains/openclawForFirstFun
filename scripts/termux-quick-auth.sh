...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/firstclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "FirstClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/firstclaw/scripts/mobile-reauth.sh'" --id firstclaw-auth
        ;;
    *)
