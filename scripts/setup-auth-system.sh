#!/bin/bash
# Setup FirstClaw Auth Management System
...
echo "=== FirstClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'firstclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FirstClaw message: Send warning via FirstClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/firstclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/firstclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/firstclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now firstclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status firstclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
