 ...
 * `firstclaw update`. Use it to verify the completion installation flow
...
\`firstclaw update\`, \`firstclaw doctor\`, and \`firstclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# FirstClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`firstclaw completion --install\` later to enable.`));
    return;
  }
