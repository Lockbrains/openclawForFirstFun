...
ZIP=${1:?"Usage: $0 FirstClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/firstclaw/firstclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^FirstClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/firstclaw/firstclaw/releases/download/v${VERSION}/"}
