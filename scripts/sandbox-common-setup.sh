...
BASE_IMAGE="${BASE_IMAGE:-firstclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-firstclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=firstclaw.sandbox=1)
NOTE
