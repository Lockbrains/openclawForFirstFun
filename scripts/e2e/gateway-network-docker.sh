...
IMAGE_NAME="firstclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="firstclaw-net-e2e-$$"
GW_NAME="firstclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "FIRSTCLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "FIRSTCLAW_SKIP_CHANNELS=1" \
	  -e "FIRSTCLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "FIRSTCLAW_SKIP_CRON=1" \
	  -e "FIRSTCLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
