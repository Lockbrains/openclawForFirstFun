...
import type { FirstClawConfig } from "../src/config/config.js";
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: FirstClawConfig) => {
    ...
    resolveAccount: (cfg: FirstClawConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: FirstClawConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
