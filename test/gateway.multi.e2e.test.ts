...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `firstclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".firstclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          FIRSTCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        FIRSTCLAW_GATEWAY_TOKEN: inst.gatewayToken,
        FIRSTCLAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          FIRSTCLAW_GATEWAY_PORT: String(gwA.port),
          FIRSTCLAW_GATEWAY_TOKEN: gwA.gatewayToken,
          FIRSTCLAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          FIRSTCLAW_GATEWAY_PORT: String(gwB.port),
          FIRSTCLAW_GATEWAY_TOKEN: gwB.gatewayToken,
          FIRSTCLAW_GATEWAY_PASSWORD: "",
        }),
