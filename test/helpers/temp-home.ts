...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("FIRSTCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.FIRSTCLAW_STATE_DIR = path.join(base, ".firstclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "firstclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".firstclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
