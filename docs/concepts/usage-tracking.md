...
- `/usage cost` in chats: local cost summary aggregated from FirstClaw session logs.
- CLI: `firstclaw status --usage` prints a full per-provider breakdown.
- CLI: `firstclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
