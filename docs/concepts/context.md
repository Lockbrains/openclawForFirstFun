...
read_when:
  - You want to understand what “context” means in FirstClaw
...
“Context” is **everything FirstClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (FirstClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How FirstClaw builds the system prompt

The system prompt is **FirstClaw-owned** and rebuilt each run. It includes:
...
By default, FirstClaw injects a fixed set of workspace files (if present):
