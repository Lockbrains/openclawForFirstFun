...
FirstClaw features that can generate provider usage or paid API calls.
...
- `firstclaw status --usage` and `firstclaw channels list` show provider **usage windows**
...
FirstClaw can pick up credentials from:
...
- `firstclaw status --usage`
- `firstclaw models status --json`
...
`firstclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
