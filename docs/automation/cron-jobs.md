...
- Jobs persist under `~/.firstclaw/cron/` so restarts don’t lose schedules.
...
firstclaw cron list
firstclaw cron run <job-id>
firstclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.firstclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `firstclaw cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, FirstClaw defaults to `announce`.
...
- Job store: `~/.firstclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.firstclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.firstclaw/cron/jobs.json",
...
firstclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
firstclaw cron edit <jobId> --agent ops
firstclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
firstclaw cron run <jobId>
firstclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
firstclaw cron edit <jobId> \
...
```bash
firstclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`firstclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `FIRSTCLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- FirstClaw applies exponential retry backoff for recurring jobs after consecutive errors:
