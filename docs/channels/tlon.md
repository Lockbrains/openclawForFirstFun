...
Tlon is a decentralized messenger built on Urbit. FirstClaw connects to your Urbit ship and can
...
```bash
firstclaw plugins install @firstclaw/tlon
```

Local checkout (when running from a git repo):

```bash
firstclaw plugins install ./extensions/tlon
```
...
Use these with `firstclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FirstClaw replies in-thread.
