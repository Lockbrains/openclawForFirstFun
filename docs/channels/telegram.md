...
```
    ...
    - or read `chat.id` from `firstclaw logs --follow`
...
- DM messages can carry `message_thread_id`; FirstClaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    FirstClaw can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, FirstClaw retries as plain text.
...
```
    ...
    - `~/.firstclaw/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, FirstClaw enqueues system events like:
...
```bash
firstclaw message send --channel telegram --target 123456789 --message "hi"
firstclaw message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `firstclaw channels status` warns when config expects unmentioned group messages.
    - `firstclaw channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `firstclaw logs --follow` for skip reasons
