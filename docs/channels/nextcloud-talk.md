...
```bash
firstclaw plugins install @firstclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
firstclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "FirstClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `firstclaw pairing list nextcloud-talk`
  - `firstclaw pairing approve nextcloud-talk <CODE>`
