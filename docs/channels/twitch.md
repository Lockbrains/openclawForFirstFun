...
read_when:
  - Setting up Twitch chat integration for FirstClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. FirstClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
firstclaw plugins install @firstclaw/twitch
```

Local checkout (when running from a git repo):

```bash
firstclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `FIRSTCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "firstclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use FIRSTCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
FIRSTCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
