...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects FirstClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
firstclaw plugins install @firstclaw/feishu
```

Local checkout (when running from a git repo):

```bash
firstclaw plugins install ./extensions/feishu
```
...
If you just installed FirstClaw, run the wizard:
...
2. Configuring app credentials in FirstClaw
...
1. You already ran `firstclaw channels add` for Feishu
2. The gateway is running (`firstclaw gateway status`)
...
```bash
firstclaw pairing approve feishu <CODE>
```
...
2. Run `firstclaw logs --follow` and look for `chat_id`
...
2. Run `firstclaw logs --follow` and look for `open_id`
...
| Command                     | Description                   |
| --------------------------- | ----------------------------- |
| `firstclaw gateway status`  | Show gateway status           |
| `firstclaw gateway install` | Install/start gateway service |
| `firstclaw gateway stop`    | Stop gateway service          |
| `firstclaw gateway restart` | Restart gateway service       |
| `firstclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `firstclaw logs --follow`
...
5. Ensure the gateway is running: `firstclaw gateway status`
6. Check logs: `firstclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.firstclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.firstclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
