...
Matrix is an open, decentralized messaging protocol. FirstClaw connects as a Matrix **user**
...
```bash
firstclaw plugins install @firstclaw/matrix
```

Local checkout (when running from a git repo):

```bash
firstclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `firstclaw plugins install @firstclaw/matrix`
   - From a local checkout: `firstclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: FirstClaw calls the same
     login endpoint, stores the access token in `~/.firstclaw/credentials/matrix/credentials.json`,
...
- On first connection, FirstClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  FirstClaw logs a warning.
...
`~/.firstclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `firstclaw pairing list matrix`
  - `firstclaw pairing approve matrix <CODE>`
...
- On startup, FirstClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
