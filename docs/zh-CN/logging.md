...
你可以在 `~/.firstclaw/firstclaw.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/firstclaw.log"
  }
}
```
...
```bash
firstclaw channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.firstclaw/firstclaw.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/firstclaw/firstclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- FirstClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
FIRSTCLAW_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `firstclaw plugins enable diagnostics-otel` 启用插件。
...
- `firstclaw.tokens`（计数器，属性：`firstclaw.token`、`firstclaw.channel`、`firstclaw.provider`、`firstclaw.model`）
- `firstclaw.cost.usd`（计数器，属性：`firstclaw.channel`、`firstclaw.provider`、`firstclaw.model`）
- `firstclaw.run.duration_ms`（直方图，属性：`firstclaw.channel`、`firstclaw.provider`、`firstclaw.model`）
- `firstclaw.context.tokens`（直方图，属性：`firstclaw.context`、`firstclaw.channel`、`firstclaw.provider`、`firstclaw.model`）

消息流：

- `firstclaw.webhook.received`（计数器，属性：`firstclaw.channel`、`firstclaw.webhook`）
- `firstclaw.webhook.error`（计数器，属性：`firstclaw.channel`、`firstclaw.webhook`）
- `firstclaw.webhook.duration_ms`（直方图，属性：`firstclaw.channel`、`firstclaw.webhook`）
- `firstclaw.message.queued`（计数器，属性：`firstclaw.channel`、`firstclaw.source`）
- `firstclaw.message.processed`（计数器，属性：`firstclaw.channel`、`firstclaw.outcome`）
- `firstclaw.message.duration_ms`（直方图，属性：`firstclaw.channel`、`firstclaw.outcome`）

队列 + 会话：

- `firstclaw.queue.lane.enqueue`（计数器，属性：`firstclaw.lane`）
- `firstclaw.queue.lane.dequeue`（计数器，属性：`firstclaw.lane`）
- `firstclaw.queue.depth`（直方图，属性：`firstclaw.lane` 或 `firstclaw.channel=heartbeat`）
- `firstclaw.queue.wait_ms`（直方图，属性：`firstclaw.lane`）
- `firstclaw.session.state`（计数器，属性：`firstclaw.state`、`firstclaw.reason`）
- `firstclaw.session.stuck`（计数器，属性：`firstclaw.state`）
- `firstclaw.session.stuck_age_ms`（直方图，属性：`firstclaw.state`）
- `firstclaw.run.attempt`（计数器，属性：`firstclaw.attempt`）

### 导出的 span（名称 + 关键属性）

- `firstclaw.model.usage`
  - `firstclaw.channel`、`firstclaw.provider`、`firstclaw.model`
  - `firstclaw.sessionKey`、`firstclaw.sessionId`
  - `firstclaw.tokens.*`（input/output/cache_read/cache_write/total）
- `firstclaw.webhook.processed`
  - `firstclaw.channel`、`firstclaw.webhook`、`firstclaw.chatId`
- `firstclaw.webhook.error`
  - `firstclaw.channel`、`firstclaw.webhook`、`firstclaw.chatId`、`firstclaw.error`
- `firstclaw.message.processed`
  - `firstclaw.channel`、`firstclaw.outcome`、`firstclaw.chatId`、`firstclaw.messageId`、`firstclaw.sessionKey`、`firstclaw.sessionId`、`firstclaw.reason`
- `firstclaw.session.stuck`
  - `firstclaw.state`、`firstclaw.ageMs`、`firstclaw.queueDepth`、`firstclaw.sessionKey`、`firstclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `firstclaw doctor`。
