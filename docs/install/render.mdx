...
Deploy FirstClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/firstclaw/firstclaw)
...
repository configures everything needed to run FirstClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: FIRSTCLAW_STATE_DIR
        value: /data/.firstclaw
      - key: FIRSTCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: FIRSTCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/firstclaw`.
...
If you use the original FirstClaw repository, Render will not auto-deploy your FirstClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For FirstClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any FirstClaw host.
