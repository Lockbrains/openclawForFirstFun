...
Deploy FirstClaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/firstclaw` — Control UI
...
- `FIRSTCLAW_STATE_DIR=/data/.firstclaw` (recommended)
- `FIRSTCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `FIRSTCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your FirstClaw state + workspace so you can migrate to another host without losing config or memory.
