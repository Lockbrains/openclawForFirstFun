...
read_when:
  - Designing workspace memory (~/.firstclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep FirstClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.firstclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.firstclaw/workspace` as canonical human-readable memory.
...
```
~/.firstclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in FirstClaw**, but keep a separable core library.

### Why integrate into FirstClaw?

- FirstClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `firstclaw memory recall "…" --k 25 --since 30d`
  - `firstclaw memory reflect --since 7d`
...
Pragmatic take for `~/.firstclaw/workspace`:
