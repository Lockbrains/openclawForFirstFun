...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.firstclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.firstclaw/node.json` (node id + pairing token).
...
Path: `~/.firstclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.firstclaw/exec-approvals.sock",
...
- Unix socket at `~/.firstclaw/exec-approvals.sock` (0600).
