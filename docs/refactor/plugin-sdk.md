...
- Publish as `firstclaw/plugin-sdk` (or export from core under `firstclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `FirstClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: FirstClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: FirstClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: FirstClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: FirstClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: FirstClawConfig): string;
  };
};
```
...
- Introduce `firstclaw/plugin-sdk`.
- Add `api.runtime` to `FirstClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `firstclawRuntime: ">=2026.2.0"`).
