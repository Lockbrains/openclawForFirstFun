...
read_when:
  - You need detailed behavior for firstclaw onboard
...
This page is the full reference for `firstclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.firstclaw/firstclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `firstclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `firstclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `firstclaw health`.
    - `firstclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.firstclaw/.env` so launchd can read it.
...
- OAuth credentials: `~/.firstclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.firstclaw/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.firstclaw/credentials/oauth.json` (or `$FIRSTCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`firstclaw onboard`](/cli/onboard)
