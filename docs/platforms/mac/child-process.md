...
service via the external `firstclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`FIRSTCLAW_PROFILE`; legacy `com.firstclaw.*` is supported).
