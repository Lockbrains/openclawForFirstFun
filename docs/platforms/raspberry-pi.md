---
summary: "FirstClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up FirstClaw on a Raspberry Pi
  - Running FirstClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# FirstClaw on Raspberry Pi

## Goal

Run a persistent, always-on FirstClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most FirstClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled firstclaw

# Enable if not
sudo systemctl enable firstclaw

# Start on boot
sudo systemctl start firstclaw
```
...
```bash
# Check logs
journalctl -u firstclaw --no-pager -n 100

# Common fix: rebuild
cd ~/firstclaw  # if using hackable install
npm run build
sudo systemctl restart firstclaw
```
