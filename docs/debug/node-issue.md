...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FirstClaw
...
Running FirstClaw via Node with `tsx` fails at startup with:

```
[firstclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `firstclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch firstclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node firstclaw.mjs status` works on Node 25.
