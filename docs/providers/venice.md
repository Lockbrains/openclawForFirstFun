---
summary: "Use Venice AI privacy-focused models in FirstClaw"
read_when:
  - You want privacy-focused inference in FirstClaw
...
```bash
firstclaw onboard --auth-choice venice-api-key
```
...
```bash
firstclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, FirstClaw shows all available Venice models. Pick based on your needs:
...
```bash
firstclaw models set venice/claude-opus-45
firstclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
firstclaw models list | grep venice
```

## Configure via `firstclaw configure`

1. Run `firstclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
FirstClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
firstclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
firstclaw chat --model venice/claude-opus-45

# Use uncensored model
firstclaw chat --model venice/venice-uncensored

# Use vision model with image
firstclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
firstclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `firstclaw models list` to see currently available models. Some models may be temporarily offline.
