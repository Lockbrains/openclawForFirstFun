---
summary: "Use MiniMax M2.1 in FirstClaw"
read_when:
  - You want MiniMax models in FirstClaw
...
```bash
firstclaw plugins enable minimax-portal-auth  # skip if already loaded.
firstclaw gateway restart  # restart if gateway is already running
firstclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/firstclaw/firstclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `firstclaw.json`:
...
- Use `firstclaw models list` and `firstclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `firstclaw configure` and selecting **MiniMax M2.1**, or
