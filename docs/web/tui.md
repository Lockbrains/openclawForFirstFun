...
```bash
firstclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `firstclaw tui --deliver`
...
- Check the Gateway logs: `firstclaw logs --follow`.
- Confirm the agent can run: `firstclaw status` and `firstclaw models status`.
...
- No agents in picker: check `firstclaw agents list` and your routing config.
