...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/firstclaw`)
...
If the page fails to load, start the Gateway first: `firstclaw gateway`.
...
you revoke it with `firstclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.firstclaw/firstclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. FirstClaw
...
FirstClaw **blocks** Control UI connections without device identity.
