...
- During heartbeat runs, FirstClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `firstclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, FirstClaw skips the heartbeat run entirely (no model call).
...
```bash
firstclaw system event --text "Check for urgent follow-ups" --mode now
```
