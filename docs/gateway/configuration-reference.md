...
description: "Complete field-by-field reference for ~/.firstclaw/firstclaw.json"
---

# Configuration Reference

Every field available in `~/.firstclaw/firstclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — FirstClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `firstclaw doctor` into `whatsapp/default`.
...
Mattermost ships as a plugin: `firstclaw plugins install @firstclaw/mattermost`.
...
FirstClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@firstclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `firstclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, FirstClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, FirstClaw assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.firstclaw/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.firstclaw/workspace",
        agentDir: "~/.firstclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@firstclaw"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:firstclaw`  | All built-in tools (excludes provider plugins)                                           |
...
FirstClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.firstclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `FIRSTCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `firstclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `firstclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `firstclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `firstclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `firstclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `firstclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.firstclaw/extensions`, `<workspace>/.firstclaw/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or FIRSTCLAW_GATEWAY_PASSWORD
      allowTailscale: true,
    },
    tailscale: {
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `FIRSTCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.firstclaw-dev` + port `19001`), `--profile <name>` (uses `~/.firstclaw-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-firstclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `FIRSTCLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    port: 18793,
    liveReload: true,
    // enabled: false, // or FIRSTCLAW_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__firstclaw__/a2ui/`.
...
- Hostname defaults to `firstclaw`. Override with `FIRSTCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.firstclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `firstclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.firstclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${FIRSTCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.firstclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/firstclaw/firstclaw-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `firstclaw doctor --fix` can strip unknown keys).
