...
FirstClaw can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere FirstClaw can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, FirstClaw defaults
...
TTS config lives under `messages.tts` in `firstclaw.json`.
...
- If `provider` is **unset**, FirstClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.firstclaw/settings/tts.json`, override with `FIRSTCLAW_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, FirstClaw retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, FirstClaw:
...
Discord note: `/tts` is a built-in Discord command, so FirstClaw registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from FirstClaw
```
