---
summary: "Agent tool surface for FirstClaw (browser, canvas, nodes, message, cron) replacing legacy `firstclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `firstclaw-*` skills
title: "Tools"
---

# Tools (FirstClaw)

FirstClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `firstclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `firstclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, FirstClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:firstclaw`: all built-in FirstClaw tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`firstclaw node run`).
...
- Requires a Brave API key (recommended: `firstclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated FirstClaw-managed browser.
...
- Quick smoke: `firstclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `firstclaw gateway` restart in-place)
