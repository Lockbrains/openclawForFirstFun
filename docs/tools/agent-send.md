---
summary: "Direct `firstclaw agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `firstclaw agent` (direct agent runs)

`firstclaw agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `firstclaw message --target`).
...
```bash
firstclaw agent --to +15555550123 --message "status update"
...
firstclaw agent --to +15555550123 --message "Summon reply" --deliver
