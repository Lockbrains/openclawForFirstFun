...
- **gateway host** → `firstclaw` process on the gateway machine
...
`~/.firstclaw/exec-approvals.json` directly.

CLI: `firstclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
