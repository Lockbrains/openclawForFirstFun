---
summary: "Chrome extension: let FirstClaw drive your existing Chrome tab"
read_when:
...
The FirstClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate firstclaw-managed Chrome profile.
...
FirstClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the FirstClaw release (npm package) as static files. There is no separate “build” step.

After upgrading FirstClaw:

- Re-run `firstclaw browser extension install` to refresh the installed files under your FirstClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

FirstClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want FirstClaw to control.
...
- `ON`: attached; FirstClaw can drive that tab.
...
Debugging: `firstclaw sandbox explain`
...
`firstclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `firstclaw browser extension install` first to copy the extension to a stable location under your FirstClaw state directory.
...
- **This is not isolated** like the dedicated firstclaw-managed profile.
