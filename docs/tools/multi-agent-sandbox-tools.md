...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `firstclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `firstclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=firstclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${FIRSTCLAW_STATE_DIR:-$HOME/.firstclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
