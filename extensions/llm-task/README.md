...
`firstclaw.invoke --each`) without adding new FirstClaw code per workflow.
...
This extension depends on FirstClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** FirstClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.firstclaw/extensions` as a standalone plugin directory.
