...
declare global {
  interface Window {
    __FIRSTCLAW_ASSISTANT_NAME__?: string;
    __FIRSTCLAW_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__FIRSTCLAW_ASSISTANT_NAME__,
    avatar: window.__FIRSTCLAW_ASSISTANT_AVATAR__,
  });
}
