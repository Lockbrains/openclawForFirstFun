...
import { FirstClawApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = FirstClawApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("firstclaw-app") as FirstClawApp;
...
beforeEach(() => {
  FirstClawApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__FIRSTCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  FirstClawApp.prototype.connect = originalConnect;
  window.__FIRSTCLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/firstclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/firstclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/firstclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__FIRSTCLAW_CONTROL_UI_BASE_PATH__ = "/firstclaw";
    const app = mountApp("/firstclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/firstclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/firstclaw/sessions");
  });
