# FirstClaw ACP Bridge

This document describes how the FirstClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`firstclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
FirstClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a FirstClaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `firstclaw acp` over stdio.

Example config:

```bash
firstclaw config set gateway.remote.url wss://gateway-host:18789
firstclaw config set gateway.remote.token <token>
```

Example run:

```bash
firstclaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
firstclaw acp --session agent:main:main
firstclaw acp --session agent:design:main
firstclaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “FirstClaw ACP” to start a thread.

## Execution Model

- ACP client spawns `firstclaw acp` and speaks ACP messages over stdio.
...
```bash
firstclaw acp --session agent:main:main
firstclaw acp --session-label "support inbox"
firstclaw acp --reset-session
```
...
`firstclaw acp` resolves the Gateway URL and auth from CLI flags or config:
